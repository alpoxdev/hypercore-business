# skills/image-generation 통합 실행 가이드

`korean-product-detail-page`는 전략 / 카피 / 컷 기획에서 멈추지 않는다. 사용자가 상세페이지를 “이미지랑 같이” 원하면, 이 스킬이 실제 이미지 생성·편집·검증·아카이브까지 수행한다. 단, 이미지 제작 방식은 새로 만들지 않고 로컬 `skills/image-generation/` 폴더의 검증된 규칙을 참고해 그대로 적용한다.

## 역할 분리

- `korean-product-detail-page`: 한국형 상세페이지 리서치, 전략, 카피, 섹션 구성, 컷 기획, 이미지 생성 실행 관리, 최종 상세페이지 패키징을 책임진다.
- `skills/image-generation`: 실제 이미지 프롬프트 구조, 자연스러움 규칙, `gpt-image-2` 실행 계약, 시각 검증, 아카이브 규칙을 제공하는 로컬 참고/실행 규칙이다.
...
이미지 생성이 포함된 작업에서는 순서대로 읽는다:

1. `skills/image-generation/SKILL.md` — 이미지 생성 전체 실행 계약
2. `skills/image-generation/rules/natural-image-workflow.md` 또는 `.ko.md` — 자연스러운 이미지 제작/검수 규칙
3. `skills/image-generation/references/json-prompt-best-practices.md` 또는 `.ko.md` — English JSON prompt 구조와 리뷰 게이트
4. `skills/image-generation/scripts/archive-generated-images.mjs` — 생성 파일 아카이브 helper
...
4. `skills/image-generation`의 JSON prompt schema로 변환한다.
...
9. 최종 이미지를 `.hypercore/image-generation/<topic-slug>/`에 `prompt.json`, `image1.*`, `image2.*` 형태로 보존한다.
...
```json
{
  ...
  "image_generation_reference": "Convert this to the English JSON prompt schema in skills/image-generation and generate/edit the actual asset."
}
```

## 변환 규칙

- 이미지 모델에 전달되는 창작 지시는 `skills/image-generation` 규칙에 따라 영어 JSON으로 작성한다.
...
- 이미지 생성 모델/경로/사이즈/품질/아카이브는 `skills/image-generation/SKILL.md`의 실행 계약을 따른다.
...
3. 결과를 실제로 시각 검증한다.
4. `skills/image-generation/scripts/archive-generated-images.mjs`를 사용하거나 동일 규칙으로 `.hypercore/image-generation/<topic-slug>/prompt.json`과 `image1.*`, `image2.*`를 보존한다.
...
- 상세페이지 산출물 경로
- 이미지 아카이브 경로
- 사용한 `skills/image-generation` 파일/규칙
