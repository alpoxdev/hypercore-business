...
4. Read `references/browser-link-research.ko.md` when the user supplies URLs or existing product/reference pages.
5. Read `references/section-templates.ko.md` when choosing the product detail page structure.
6. Read `references/image-direction.ko.md` when producing image/cut briefs.
7. Read `references/image-generation-integration.ko.md` when the user wants actual generated/edited image files or prompt-ready JSON.
8. For real image creation, read `skills/image-generation/SKILL.md` plus the specific local support files named in the integration reference, then continue through generation, validation, and archive.
9. Read `rules/platform-compliance.md` before finalizing any SmartStore/Cafe24/Gmarket/Auction-oriented output.
10. Use `scripts/validate-korean-product-detail-page-skill.mjs` after editing this skill.
