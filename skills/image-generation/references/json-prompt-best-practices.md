...
1. **Generation settings**: model, task, size, quality, format, destination, canonical save path, archive directory, and preview HTML path.
...
- Put `model`, `task`, `size`, `quality`, `format`, destination, `archive_dir`, `save_path`, `prompt_path`, and `preview_html_path` under `generation_settings` / `output`.
