...
5. **Convert requirements into an English JSON prompt.** Use the schema below. Keep prompt-facing values in English, preserve exact requested text verbatim, and encode assumptions explicitly.
6. **Review the JSON prompt before generation.** Check that it is valid JSON, specific to the situation, coherent, non-contradictory, safe, and compatible with `gpt-image-2`. Fix the JSON before generation if any review item fails.
...
The prompt must be created as JSON first. Use valid JSON only: double-quoted keys/strings, no comments, no trailing commas.
...
- If the JSON mixes multiple capture stories, split into variants or choose the strongest single story before generation.
- If research anchors are missing for factual/current/specialized scenes, research first or mark why research was unnecessary.
...
- `references/gpt-image-2-research.md`: source-backed research synthesis and links.
- `.hypercore/research/2026-04-29-image-generation-naturalism.md`: full research report saved for reuse.
